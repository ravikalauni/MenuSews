...
export default function BarSidebar({ activeView, onViewChange, onLogout, isOpen, onClose }: Props) {
  const menuItems = [
    { id: 'current', label: 'Active Orders', icon: '🍷' },
    ...
    <>
      ...
      `}>
        ...
        <div className="p-6 border-b border-gray-100 flex items-center gap-3">
           ...
           <div>
              <h1 className="text-xl font-black text-gray-800 leading-none">Bar</h1>
              <p className="text-[10px] font-bold text-gray-400 mt-1 uppercase tracking-wider">Drinks Station</p>
           </div>
        </div>

        {/* Navigation Items */}
        <div className="flex-1 overflow-y-auto py-6 px-4 space-y-2">
           <p className="px-4 text-xs font-bold text-gray-400 uppercase mb-2">Queue</p>
           {menuItems.map(item => (
             <button
               key={item.id}
